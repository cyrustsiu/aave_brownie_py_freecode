...
def main():
    ...
    tx = lending_pool.deposit(erc20_address, amount, account.address, 0, {
        "from": account})  # referal code don't work anymore
    tx.wait(1)
    print("Deposited!")
    # Borrow...how much?
    ...
    # multiply as a buffer, for better health factor
    amount_dai_to_borrow = (1 / dai_eth_price) * (borrowable_eth * 0.95)
    print(f"We are going to borrow {amount_dai_to_borrow} DAI.")
    dai_address = config["networks"][network.show_active()]["dai_token"]
    borrow_tx = lending_pool.borrow(dai_address, Web3.toWei(
        amount_dai_to_borrow, "ether"), 1, 0, account.address, {"from": account})  # hard coded 1: fixed rate
    borrow_tx.wait(1)
